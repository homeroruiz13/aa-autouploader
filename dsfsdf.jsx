...
// --- Helper function to process regular mockups (improved) ---
function processMockup(mockupPath, patternPath, outputFolder, baseName) {
    try {
        ...
        // Wait for document to open completely
        $.sleep(1000);
        ...
        for (var j = 0; j < targetLayers.length; j++) {
            try {
                ...
                $.sleep(500);  // Wait for pattern to open
                ...
                $.sleep(500);  // Wait for resize to complete
        ...
        try {
            ...
            logMessage("Exported: " + outputPNGFileName);
        } catch (e) {
            logMessage("Error exporting " + finalFileName + ": " + e.message);
        }

        mockupDoc.close(SaveOptions.DONOTSAVECHANGES);
        $.sleep(1000);  // Wait for document to close completely
...
// === Improved Rolled Image Processing Function ===
