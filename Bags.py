...
// Generated by Bags.py on {time.ctime()}
...
def read_csv_and_download_images(csv_path):
    """Read image URLs and names from a CSV file and download the images"""
    logging.info(f"Reading CSV file: {csv_path}")
    
    # Create a timestamp for folder naming
    today_date = datetime.datetime.now().strftime("%Y-%m-%d")
    main_folder_name = f"{today_date}_Downloaded_Images"
    
    # Get the downloads folder path and create the download directory
    downloads_path = Path.home() / "Downloads"
    download_dir = downloads_path / main_folder_name
    ...
    try:
        with open(csv_path, 'r', newline='', encoding='utf-8') as csvfile:
            ...
            # Find the indices of the URL and name columns
            url_idx = 0  # Default: first column
            name_idx = 1  # Default: second column
            
            # Find column indices if headers exist
            if headers:
            ...
            for row in csv_reader:
                if len(row) > max(url_idx, name_idx):
                    url = row[url_idx].strip()
                    name = row[name_idx].strip()
                    
                    if url and name:
                        # Clean the filename and ensure it has a valid extension
                        # Default to .jpg if no extension is found in the URL
                        clean_name = "".join(c for c in name if c.isalnum() or c in " .-_").strip()
                        clean_name = clean_name.replace(" ", "_")
                        
                        # Get the extension from URL or default to .jpg
                        url_parts = url.split('.')
                        if len(url_parts) > 1 and url_parts[-1].lower() in ['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp', 'tiff']:
                            file_ext = f".{url_parts[-1].lower()}"
                        else:
                            file_ext = ".jpg"
                        
                        # Make sure the filename has the extension
                        if not clean_name.lower().endswith(file_ext):
                            clean_name += file_ext
                        
                        # Create the full save path
                        save_path = download_dir / clean_name
                        
                        logging.info(f"Downloading image: {url} as {clean_name}")
                        ...
                        logging.warning(f"Skipping row with empty URL or name: {row}")
                else:
                    logging.warning(f"Skipping row with insufficient columns: {row}")
    
    except Exception as e:
        logging.error(f"Error reading CSV or downloading images: {e}")
        logging.exception("Exception details:")
        print(f"Error: {e}")
    
    return downloaded_images, download_dir

def process_from_csv():
    """Process images from a CSV file with URLs instead of JSON"""
    logging.info("Starting CSV-based image processing")
    
    try:
        # Ask for the CSV file path
        csv_path = input("Enter the path to your CSV file: ")
        csv_path = Path(csv_path.strip().strip('"').strip("'"))  # Remove quotes and extra spaces
        
        # Check if the file exists with exact path
        if not csv_path.exists():
            # If not found, try replacing spaces with underscores and vice versa
            alt_path1 = Path(str(csv_path).replace(" ", "_"))
            alt_path2 = Path(str(csv_path).replace("_", " "))
            
            if alt_path1.exists():
                csv_path = alt_path1
                logging.info(f"Using alternative path with underscores: {csv_path}")
            elif alt_path2.exists():
                csv_path = alt_path2
                logging.info(f"Using alternative path with spaces: {csv_path}")
            else:
                error_msg = f"Error: CSV file not found at {csv_path}"
                logging.error(error_msg)
                print(error_msg)
                return
        
        # Read the CSV and download the images
        downloaded_images, download_dir = read_csv_and_download_images(csv_path)
        
        if not downloaded_images:
            error_msg = "No images were downloaded successfully"
            logging.error(error_msg)
            print(error_msg)
            return
        ...
        # Process each downloaded image to create tiled versions
...
if __name__ == "__main__":
    logging.info("=== Starting Bags.py ===")
    try:
        result = process_from_csv()
        if result:
